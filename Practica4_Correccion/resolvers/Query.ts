  ...
  const Query = {
    ...
    getMyTasks: async (parent: any, args: any, ctx: IContext) => {
      try {
        ...
        const tasksCollection = db.collection<TaskSchema>("Tasks");
        const tasks = await tasksCollection.find({$or: [{ assignee: email }, { reporter: email }]});
        const result = tasks.map((t) => {
          return {
            ...t,
            date: t.date.toString(),
    ...
    getMyOpenTasks: async (parent: any, args: any, ctx: IContext) => {
      try {
        ...
        const email: string = ctx.user.email;
        const tasksCollection = db.collection<TaskSchema>("Tasks");
        const tasks = await tasksCollection.find({$and: [{status: { $ne: "DONE" } }, { $or: [{ assignee: email }, { reporter: email }] }]});
        const result = tasks.map((t) => {
          return {
            ...t,
            date: t.date.toString(),
    ...
    getTaskByStatus: async (parent: any, args: IGetTaskByStatusArgs, ctx: IContext) => {
      try {
    ...
    getTaskByDate: async (parent: any, args: IGetTaskByDateArgs, ctx: IContext) => {
      try {
        const db: Database = ctx.db;
        const tasksCollection = db.collection<TaskSchema>("Tasks");
