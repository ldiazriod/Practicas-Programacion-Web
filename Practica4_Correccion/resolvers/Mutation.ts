...
const Mutation = {
    addTask: async (parent: any, args: IAddTaskArgs, ctx: IContext): Promise<boolean> => {
        try{
            ...
            if(found){
                throw new GQLError("Task id already exists")
            }

            const {name, description, id, status, year, month, day, assignee} = args.task
            const task = {
                name,
                description,
                id,
    ...
    addUser: async (parent: any, args: IAddUserArgs, ctx: IContext): Promise<boolean> => {
        try{
            const db: Database = ctx.db;
            ...
            if(found){
                throw new GQLError("User with email already exists")
            }
    ...
    updateTask: async (parent: any, args: IUpdateTaskArgs, ctx: IContext): Promise<Boolean> => {
        try {
            ...
            if(!found){
                throw new GQLError("Task with id does not exist");
            }
    ...
    completeTask: async (parent: any, args: {id: string}, ctx: IContext): Promise<boolean> => {
        try{
            const db: Database = ctx.db;
            const tasksCollection: Collection<TaskSchema> = db.collection<TaskSchema>("Task")
            ...
            if(!found){
                throw GQLError("Task with id not found");
            }
    ...
    startTask: async (parent: any, args: {id: string}, ctx: IContext): Promise<Boolean> => {
        try{
            const db: Database = ctx.db;
            const tasksCollection: Collection<TaskSchema> = db.collection<TaskSchema>("Tasks")

            const found = await tasksCollection.updateOne({id: args.id}, {status: "DOING"})
            return true;
        }catch (e) {
            throw new GQLError(e)
        }
    ...
    signin: async (parent: any, args: {email: string, password: string}, ctx: IContext): Promise<boolean> => {
        try{
            const exists = await ctx.db.collection<UserSchema>("Users").findOne({email: args.email});
            if(exists){
                throw new GQLError(`User with email ${args.email} already exists`)
            }
            await ctx.db.collection<UserSchema>("Users").insertOne({email: args.email, password: args.password, token: ""})
            return true;
        }catch (e) {
            throw new GQLError(e)
        }
    ...
    login: async (parent: any, args: {email: string, password: string}, ctx: IContext): Promise<string> => {
        try{
            const findOne = await ctx.db.collection<UserSchema>("Users").findOne({email: args.email, password: args.password})
            if(findOne){
                const token = v4.generate();
                await ctx.db.collection<UserSchema>("Users").updateOne({email: args.email}, {$set: {token}})
                return token;
            }else{
    ...
    logout: async (parent: any, args: {}, ctx: IContext): Promise<boolean> => {
        try{
            const findOne = await ctx.db.collection<UserSchema>("Users").findOne({email: ctx.user.email, token: ctx.user.token})
            if(findOne){
                await ctx.db.collection<UserSchema>("Users").updateOne({email: ctx.user.email}, {$set:{token: ""}})
                return true;
            }else{
    ...
    deleteOne: async (parent: any, args: {}, ctx: IContext): Promise<boolean> => {
        try{
            const findOne = await ctx.db.collection<UserSchema>("Users").findOne({email: ctx.user.email, token: ctx.user.token})
            if(findOne){
                await ctx.db.collection<UserSchema>("Users").deleteOne({email: ctx.user.email})
                return true;
            }else{
                throw new GQLError("Unexpected error");
            }
