...
import {
  ...
  const Task = {
    assignee: async (parent: {assignee: string}, args: any, ctx: IContext): Promise<IUser | null> => {
    ...
    reporter: async (parent: {reporter: string}, args: any, ctx: IContext): Promise<IUser | null> => {
      const db: Database = ctx.db;
      const UsersCollection = db.collection<UserSchema>("Users");
